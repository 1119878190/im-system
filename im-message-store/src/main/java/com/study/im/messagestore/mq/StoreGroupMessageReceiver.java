...
public class StoreGroupMessageReceiver {
    ...
    @RabbitListener(
            ...
            bindings = @QueueBinding(
                    // 绑定 MQ 队列
                    value = @Queue(value = Constants.RabbitConstants.StoreGroupMessage, declare = "true"),
                    // 绑定 MQ 交换机
                    exchange = @Exchange(value = Constants.RabbitConstants.StoreGroupMessage, declare = "true")
            ), concurrency = "1"
    ...
    public void onChatMessage(@Payload Message message, @Headers Map<String, Object> headers, Channel channel) throws Exception {
        ...
        try {
            ...
            doStoreGroupMessageDto.setMessageBodyEntity(messageBody);
